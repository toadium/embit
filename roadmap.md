...
**现状**：`ik.mbt` Phase 2 接口骨架，`SimpleIKSolver` 为占位映射。

**任务**：

- [ ] 实现 6-DOF 工业臂 DLS（阻尼最小二乘）逆运动学
- [ ] 实现 7-DOF 冗余臂逆运动学（零空间优化）
- [ ] 实现人形腿逆运动学（2-DOF 平面 + 4-DOF 空间）
- [ ] IK 数值稳定性：雅可比奇异点处理、关节限位约束
- [ ] 多解选择：基于种子选择最近解
...
| TD-05 | `ik.mbt:34` | IK 解算器仅骨架 | P1 |
| TD-06 | `gazebo_robot.mbt:7` | 传感器读数占位返回空数组 | P1 |
| TD-07 | `pipeline_demos.mbt:160` | 传感器噪声省略 | P2 |
| TD-08 | `pipeline_demos.mbt:198` | 上下文使用量检查占位 | P2 |
